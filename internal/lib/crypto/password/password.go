...
func GenerateHash(str, pepper string) (string, error) {
	...
	res := base64.RawStdEncoding.EncodeToString(result)
	salted := base64.RawStdEncoding.EncodeToString(salt)
...
func VerifyHash(pw, hash string) (bool, error) {
	...
	salt, err = base64.RawStdEncoding.DecodeString(hashArr[4])
	if err != nil {
		return false, logger.ErrorWrap(fmt.Errorf("invalid hash value"), "util", "Failed on getting hash")
	}
	hashed, err = base64.RawStdEncoding.DecodeString(hashArr[5])
	if err != nil {
		return false, logger.ErrorWrap(fmt.Errorf("invalid hash value"), "util", "Failed on getting hash")
	}
